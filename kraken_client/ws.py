...
class WSKrakenClient:
    ...
    def _gen_raw_subscription_payload(
        self,
        event: str, sub_name: str, reqid: int = None, pair: list[str] = None,
        sub_depth: int = None, sub_interval: int = None, sub_ratecounter: int = None,
        sub_snapshot: bool = None, sub_token: str = None
    ) -> dict:
        _locals = locals()
        del _locals['self']
        payload = {
            k: v for (k, v) in _locals.items() if not k.startswith("sub") and v is not None
        }
        payload['subscription'] = {
            k[4:]: v for (k, v) in _locals.items() if k.startswith("sub") and v is not None
        }
        return payload
    ...
    async def subscribe_own_trades(self, token: str, snapshot: bool = True, **kwargs) -> None:
        await self.auth_send(WSKrakenOutMsg(
            self._gen_raw_subscribe_payload(
                sub_name='ownTrades',
                sub_token=token,
    ...
    async def cancel_order(self, token: str, txid: list[str]):
        await self.auth_send(WSKrakenOutMsg(dict(
            event='cancelOrder',
            token=token,
            txid=txid
        )))
